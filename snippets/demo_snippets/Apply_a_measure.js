...
function getTargetLongTableFromGraph(targetLiveDash, props2get) {
    let table = [];
...
function fill(node, properties, line) {
    ...
    for (let property of properties) {
        if (property in node.tag.properties) {
            node.tag.properties[property].expression = !isNaN(+line[property]) ? +line[property] : `"${line[property]}"`
            node.tag.properties[property].value = !isNaN(+line[property]) ? +line[property] : line[property]
...
function findNode(propertyWithID, columnWithID, line) {
    let node = liveDashPanelWhatIf.masterGraph.nodes.find(node => {
        if (propertyWithID in node.tag.properties) {
            return str(node.tag.properties[propertyWithID].expression) === str(line[columnWithID])
        } else {
            return false
        }
    })
